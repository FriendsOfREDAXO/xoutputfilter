<?php
function xoutputfilterBackendCheckDup($value, $lang = '')
{
    ...
    $whereParams = ['typ' => '5', 'name' => $value, 'lang' => $clang, 'id' => $oid];
...
if ($func == 'setvalue')
{
    ...
    if ($rc['value'] <> $rc['oldvalue']) {
        if ($rc['field'] == 'name') {
            if ($rc['value'] <> $rc['oldvalue']) {
                if (!xoutputfilterBackendCheckDup($rc['value'], $rc['lang'])) {
                    $rc['error'] = 1;
                    $rc['msg'] = '[' . $rc['value'] . '] - ' . $this->i18n('xoutputfilter_backend_error_exists');
                    echo json_encode($rc);
                    exit;
                }
            }
        ...
        $whereParams = ['typ' => '5', 'id' => $rc['id']];
        $sql->setWhere('`typ` = :typ AND `id` = :id', $whereParams);
        ...
        try {
            ...
            if ($sql->getRows() == 1) {
                ...
                $rc['error'] = 1;
                $rc['msg'] = $rc['query'].'[' . $rc['value'] . '] - ' . $this->i18n('xoutputfilter_backend_error_update');
            }
...
if ($xfunc <> '') {
    ...
    if ($Values['name'] <> $oldmarker) {
        if (!xoutputfilterBackendCheckDup($Values['name'])) {
            $error .= $this->i18n('xoutputfilter_backend_error_exists') . '<br>';
        }
    }
    if ($Values['name'] == '') {
        $error .= $this->i18n('xoutputfilter_backend_error_no_name'). '<br>';
    }
    if ($Values['description'] == '') {
        $error .= $this->i18n('xoutputfilter_backend_error_no_description'). '<br>';
    }
    if ($Values['marker'] == '') {
        $error .= $this->i18n('xoutputfilter_backend_error_no_marker'). '<br>';
    }
    ...
    if (!$error) {
        ...
        if ((trim(rex_request('save', 'string', '')) == '1') and ($oid == -1)) {
            try {
                ...
                $error = $this->i18n('xoutputfilter_backend_error_update');
            }
        // übernehmen / update
        } else {
            try {
                ...
                $sql->setWhere( ['typ' => '5', 'lang' => $clang, 'id' => $oid] );
                $sql->setTable($table);
                $sql->update();
                ...
                if (rex_addon::get('xoutputfilter')->getConfig('syncfrontend')) {
                    ...
                    $whereParams = ['typ' => '5', 'oldmarker' => $oldmarker];
...
if ($func == 'add' || $func == 'edit')
{
    ...
    $fieldset = $func == 'edit' ? $this->i18n('xoutputfilter_backend_title_edit') : $this->i18n('xoutputfilter_backend_title_add');

    // Name
    $formElements = [];
    $n = [];
    $n['label'] = '<label for="name">' . htmlspecialchars_decode($this->i18n('xoutputfilter_backend_label_name')) . '</label>';
    ...
    $formElements = [];
    $n = [];
    $n['label'] = '<label for="description">' . htmlspecialchars_decode($this->i18n('xoutputfilter_backend_label_description')) . '</label>';
    ...
    $field->setLabel(htmlspecialchars_decode($this->i18n('xoutputfilter_backend_label_active')));
    ...
    $field->setLabel(htmlspecialchars_decode($this->i18n('xoutputfilter_backend_insertbefore')));
    $select = $field->getSelect();
    $select->addOption($this->i18n('xoutputfilter_backend_insertbefore0'), 0);
    $select->addOption($this->i18n('xoutputfilter_backend_insertbefore1'), 1);
    $select->addOption($this->i18n('xoutputfilter_backend_insertbefore2'), 2);
    $select->addOption($this->i18n('xoutputfilter_backend_insertbefore3'), 3);
    $select->addOption($this->i18n('xoutputfilter_backend_insertbefore4'), 4);
    $select->setSize(1);
    ...
    $n['label'] = '<label for="marker">' . htmlspecialchars_decode($this->i18n('xoutputfilter_backend_label_marker')) . '</label>';
    ...
    $n['label'] = '<label for="html">' . htmlspecialchars_decode($this->i18n('xoutputfilter_backend_label_html')) . '</label>';
    ...
    $formElements = [];
    $n = [];
    $n['label'] = '<label for="categories">' . htmlspecialchars_decode($this->i18n('xoutputfilter_backend_categories')) . '</label>';
    $n['field'] = '<input class="form-control" type="text" id="categories" name="categories" value="' . htmlspecialchars($Values['categories']) . '" />';
    $formElements[] = $n;
    ...
    $field->setLabel(htmlspecialchars_decode($this->i18n('xoutputfilter_backend_allcats')));
    $select = $field->getSelect();
    $select->addOption($this->i18n('yes'), 1);
    $select->addOption($this->i18n('no'), 0);
    $select->setSize(1);
    ...
    $field->setLabel(htmlspecialchars_decode($this->i18n('xoutputfilter_backend_once')));
    $select = $field->getSelect();
    $select->addOption($this->i18n('yes'), 1);
    $select->addOption($this->i18n('no'), 0);
    $select->setSize(1);
    ...
    $formElements = [];
    $n = [];
    $n['label'] = '<label for="excludeids">' . htmlspecialchars_decode($this->i18n('xoutputfilter_backend_excludeids')) . '</label>';
    $n['field'] = '<input class="form-control" type="text" id="excludeids" name="excludeids" value="' . htmlspecialchars($Values['excludeids']) . '" />';
    $formElements[] = $n;
    ...
    if ($func == 'edit') {
        ...
        $n = [];
        $n['field'] = '<button class="btn btn-delete" type="submit" name="delete" value="1" data-confirm="[' . htmlspecialchars($Values['name']) . '] - ' . $this->i18n('xoutputfilter_backend_really_delete'). '">' . $this->i18n('xoutputfilter_button_delete') . '</button>';
        $formElements[] = $n;
    }
...
if ($func == 'delete')
{
    ...
    echo rex_view::success($this->i18n('xoutputfilter_backend_deleted'));
    $func = '';
}
...
if ($func == 'setstatus')
{
    ...
    $msg = $status == 1 ? 'xoutputfilter_backend_status_activated' : 'xoutputfilter_backend_status_deactivated';
...
if ($func == '')
{
    ...
    if (isset($_SESSION['xoutputfilter']['backend']['filter1']) and $_SESSION['xoutputfilter']['backend']['filter1'] <> '')
    {
        ...
        $sqlfilter .= ' OR `description` like \'%'.$_SESSION['xoutputfilter']['backend']['filter1'].'%\') ';
    }
    ...
    $list->setNoRowsMessage($this->i18n('xoutputfilter_backend_nodata'));

    $list->addTableAttribute('class', 'table-striped table-hover xoutputfilter');

    $list->addTableColumnGroup(array(40, 120, '*', 300, 20, 20, 20, 20));

    $tdIcon = '<i class="rex-icon rex-icon-edit" title="' . $this->i18n('edit') . '"></i>';
    $thIcon = '<a href="' . $list->getUrl(['func' => 'add']) . '" ' . rex::getAccesskey($this->i18n('backend_add'), 'add') . ' title="' . $this->i18n('languages_add') . '"><i class="rex-icon rex-icon-add"></i></a>';
    ...
    $list->setColumnLabel('description', $this->i18n('xoutputfilter_backend_description_header'));
    ...
    $list->setColumnLabel('marker', $this->i18n('xoutputfilter_backend_marker_header'));
    ...
    $fragment = new rex_fragment();
    $fragment->setVar('title', $this->i18n('xoutputfilter_backend_table_header'), false);
